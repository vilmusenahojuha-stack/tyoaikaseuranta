...
(() => {
  ...
  const DEFAULT_CFG = {
  sheetsUrl: "https://script.google.com/macros/s/AKfycbw5ajra2MYJoQ2SuQqNfQN4XBuCc3-0EvUULWfCmABl8H60_c400xKy2WLjTcAeraKaPA/exec",
...
function getSelectedPlate(){ return normalizePlate($("plateSelect")?.value || ""); }
  ...
  // Minutes -> decimal hours string (e.g. 60 -> "1", 90 -> "1.5")
  function minutesToHoursText(min){
    const h = round2(Number(min || 0) / 60);
    // trim trailing zeros
    return String(h).replace(/\.0+$|(?<=\.[0-9]*)0+$/g, "").replace(/\.$/,"");
  }

  // Decimal hours number -> display string (trim trailing zeros)
  function hoursToText(h){
    const x = round2(Number(h || 0));
    return String(x).replace(/\.0+$|(?<=\.[0-9]*)0+$/g, "").replace(/\.$/,"");
  }
...
function normalizeRow(r) {
  ...
  return {
    ...
    dayH: Number(r.dayH || 0),
    eveH: Number(r.eveH || 0),
    nightH: Number(r.nightH || 0),
    totalH: Number(r.totalH || 0),
...
async function refreshHistoryFromSheets() {
 ...
 async function trySendEntryToSheets(entry) {
  ...
  new Date(b.timestamp || 0) - new Date(a.timestamp || 0)
	...
	const sub = `${plateTxt}
Työaika: ${hoursToText(e.totalH)} h | 
Aamu: ${hoursToText(e.dayH)} h | 
Ilta: ${hoursToText(e.eveH)} h | 
Yö: ${hoursToText(e.nightH)} h | 
...
Päiväraha: ${perDiemText(e.perDiem)}`;
      ...
      div.innerHTML = `
        ...
        <div class="meta">
          <div class="time"><b>${e.user}</b> — ${hoursToText(e.totalH)} h</div>
  ...
  function renderLive() {
    ...
    // totals from history
    const histTotal = history.reduce((sum, e) => sum + Number(e.totalH || 0), 0);
    ...
    $("liveToday").textContent = minutesToHoursText(todayMin);
    $("liveAll").textContent = hoursToText(histTotal + (todayMin/60));
  ...
  function recalcSummaryPanel() {
    ...
    $("sumTotal").textContent = minutesToHoursText(totalMin) + " h";
    $("sumDay").textContent = minutesToHoursText(adjSeg.day);
    $("sumEve").textContent = minutesToHoursText(adjSeg.eve);
    $("sumNight").textContent = minutesToHoursText(adjSeg.night);
  }
  ...
  async function approveSummary() {
    ...
    const entry = {
      ...
      dayH: round2(adjSeg.day/60),
      eveH: round2(adjSeg.eve/60),
      nightH: round2(adjSeg.night/60),
      totalH: round2(totalMin/60),
      perDiem,
      approved: true,
      approvedTs: Date.now(),
      sent: false,
