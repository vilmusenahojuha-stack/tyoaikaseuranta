...
function getSelectedPlate(){ return normalizePlate($("plateSelect")?.value || ""); }
  ...
  function minutesToDecHours(min){
    min = Math.max(0, Number(min) || 0);
    const h = min / 60;
    // 2 desimaalia, mutta siistitään turhat nollat
    let s = (Math.round(h * 100) / 100).toFixed(2);
    s = s.replace(/\.00$/, "");
    s = s.replace(/(\.\d)0$/, "$1");
    return s;
  }
...
function normalizeRow(r) {
  ...
  return {
    ...
    dayMin: Number(r.dayMin || 0),
    eveMin: Number(r.eveMin || 0),
    nightMin: Number(r.nightMin || 0),
...
async function refreshHistoryFromSheets() {
 ...
 async function trySendEntryToSheets(entry) {
  ...
  new Date(b.timestamp || 0) - new Date(a.timestamp || 0)
	...
	const sub = `${plateTxt}
Työaika: ${minutesToDecHours(e.totalMin)} | 
Päivä: ${minutesToDecHours(e.dayMin)} h | 
Ilta: ${minutesToDecHours(e.eveMin)} h | 
Yö: ${minutesToDecHours(e.nightMin)} h | 
...
Päiväraha: ${perDiemText(e.perDiem)}`;
      ...
      div.innerHTML = `
        ...
        <div class="meta">
          <div class="time"><b>${e.user}</b> — ${minutesToDecHours(e.totalMin)}</div>
  ...
  function renderLive() {
    ...
    $("liveToday").textContent = minutesToDecHours(todayMin);
    $("liveAll").textContent = minutesToDecHours(histTotal + todayMin);
  ...
  function recalcSummaryPanel() {
    ...
    $("sumTotal").textContent = minutesToDecHours(totalMin);
    $("sumDay").textContent = minutesToDecHours(adjSeg.day);
    $("sumEve").textContent = minutesToDecHours(adjSeg.eve);
    $("sumNight").textContent = minutesToDecHours(adjSeg.night);
  }
